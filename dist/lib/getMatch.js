export default async function getMatch(LeagueId, teamId) {
    const response = await fetch(`https://www.fotmob.com/api/leagues?id=${LeagueId}`);
    const data = await response.json();
    const allMatches = data.matches.allMatches;
    const totalMatches = allMatches.length;
    for (let i = totalMatches - 1; i >= 0; --i) {
        const { home, away, status } = allMatches[i];
        if (home.id != teamId && away.id != teamId)
            continue;
        if (!status.started)
            continue;
